...
class CustomRendererPlain extends \cogpowered\FineDiff\Render\Renderer
{
    public function callback($opcode, $from, $from_offset, $from_len)
    {
        if ($opcode === 'c') {
            ...
            $html = '<del class="html-diff-del">'.htmlentities(htmlentities($deletion)).'</del>';

        } else /* if ( $opcode === 'i' ) */ {
            $html = '<ins class="html-diff-ins">'.htmlentities(htmlentities(substr($from, $from_offset, $from_len))).'</ins>';
        }
